...
├── migrations/
│   ├── 001_init.sql
│   └── 002_events_event_id_unique.sql
├── .env.example
├── .gitignore
├── AI_NOTES.md
├── Dockerfile
...
in-memory cache is seeded from it at startup, so this stays correct across
restarts even before an account's next event arrives.
...
See `SOLUTION.md` for a concise explanation of:

The defects that were identified
Why the PostgreSQL idempotency strategy was selected
The considerations for scaling to higher webhook volumes

## AI disclosure

AI assistance was used during development. See `AI_NOTES.md` for what it
was used for.
